...
public class PlayerMove_M : MonoBehaviourPunCallbacks, IPunObservable {
    ...
    {
        if(GameManager_M._instance.ThisTurn() == 0 && PhotonNetwork.IsMasterClient)
        {
        ...
        if (GameManager_M._instance.ThisTurn() == 1 && !PhotonNetwork.IsMasterClient)
        {
    ...
    void Update () {
		...
		if(Input.GetMouseButtonDown(0)){
			...
			if(Physics.Raycast(ray,out hit)){
                ...
                if((hit.transform.name == "P1_M" && GameManager_M._instance.ThisTurn() == 0) || (hit.transform.name == "P2_M" && GameManager_M._instance.ThisTurn() == 1))
                {
					touch = touch +1;
